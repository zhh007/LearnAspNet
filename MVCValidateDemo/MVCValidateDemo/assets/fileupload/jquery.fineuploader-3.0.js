...
var ext_xls = ['xls', 'xlsx'];
var ext_ppt = ['ppt', 'pptx'];
var ext_zip = ['zip', 'rar'];
var ext_pdf = ['pdf'];
var ext_pic = ['jpg', 'png', 'jpeg', 'gif', 'ico'];
...
qq.FineUploaderBasic.prototype = {
    ...
    _maybeParseAndSendUploadError: function (id, fileName, response, xhr) {
        ...
        if (!response.success) {
            if (xhr && xhr.status !== 200 && !response.error) {
                ...
                if (response.is404 != undefined && response.is404) {
                    errorReason = this._error('sizeError', fileName);
                }
    ...
    _validateFile: function (file) {
        ...
        if (this._isForbidExtension(name)) {
            this._error('excludeError', name);
            return false;
        }
        ...
        else if (size && this._options.validation.totalSizeLimit && size > this._options.validation.totalSizeLimit) {
            this._error('totalSizeError', name);
            return false;
        }

        if (this._options.validation.regex && this._options.validation.regex.length > 0) {
            if (!name.match(new RegExp(this._options.validation.regex))) {
                this._error('regexError', name);
                return false;
            }
        }
        ...
        //已存在
        var exist = false;
        $(this._listElement).find(".qq-upload-file").each(function (i, o) {
            if ($(o).text() == name) {
                exist = true;
                return false;
            }
        });
        if (exist) { this._error('onExist', name); return false; }
    ...
    _error: function (code, fileName) {
        ...
        r('{totalSizeLimit}', this._formatSize(this._options.validation.totalSizeLimit));
    ...
    _isForbidExtension: function (fileName) {
        var ext = (-1 !== fileName.indexOf('.'))
            ? fileName.replace(/.*[.]/, '').toLowerCase()
            : '';
        var forbid = this._options.validation.forbidExtensions;

        if (!forbid || !forbid.length) { return false; }

        for (var i = 0; i < forbid.length; i++) {
            if (forbid[i].toLowerCase() == ext) { return true; }
        }
...
qq.FineUploader = function (o) {
    ...
    qq.extend(this._options, {
        ...
        template: '<div class="qq-uploader">' +
            ...
            (!this._options.button ? '<div class="qq-upload-button"><a href="javascript:void(0)">{uploadButtonText}</a></div>' : '') +
        ...
        //'<div class="qq-progress-bar"></div>' +
            ...
            '<span class="qq-upload-cancel" onclick="EFDeleteUploadFile(this, \'' + this._options.deleteHandler + '\', \'' + this._options.folder + '\')">{cancelButtonText}</span>' +
...
qq.extend(qq.FineUploader.prototype, {
    ...
    **/
    _find: function (parent, type) {
        if (type == "list") {
            return document.getElementById(this._element.id + "-list");
        }
    ...
    _addToList: function (id, fileName) {
        ...
        qq(fileElement).addClass(this._fileClass(fileName));
    ...
    _fileClass: function (fileName) {
        var t = fileName.split(".");
        var ext = t[t.length - 1].toLowerCase();
        if ($.inArray(ext, ext_doc) > -1) {
            return "f_doc";
        } else if ($.inArray(ext, ext_xls) > -1) {
            return "f_xls";
        } else if ($.inArray(ext, ext_ppt) > -1) {
            return "f_ppt";
        } else if ($.inArray(ext, ext_zip) > -1) {
            return "f_zip";
        } else if ($.inArray(ext, ext_pdf) > -1) {
            return "f_pdf";
        } else if ($.inArray(ext, ext_pic) > -1) {
            return "f_pic";
        }
        return "f_file";
    },
    ...
    */
    _getIframeContentJSON: function (iframe) {
        ...
        try {
            ...
            response = { success: false, is404: true, message: "" };
        }
